...
# Fit lassosum2 in each chromosome
lastout <- alply(c(1: 22), 1, function(chr){
  ...
  val_sub_str <- paste0(TEMP_DIR, "/val_sub-", 
  ...
  if (h2[2] < 0){
    ...
    beta_lassosum2 <- data.frame(df_beta_h2$rsid[df_beta_h2$rsid %in% snp_inter],
    ...
    beta_grid <- snp_lassosum2(as_SFBM(corr), df_beta_h2, ncores = 1)
    params2 <- attr(beta_grid, "grid_param")
  ...
  colnames(beta_lassosum2) <- c('rsid','a1','effect')
