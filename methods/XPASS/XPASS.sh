...
ref1=/disk/reference_pgsfusion/1kg/${ancestry1}/hm3_imp/mergeout
ref2=/disk/reference_pgsfusion/1kg/${ancestry1}/hm3_imp/mergeout

# Set pc
if [ "$ancestry1" = "EUR" ]
then
	pc1=/disk/reference_pgsfusion/1kg/${$ancestry1}/hm3_imp/${$ancestry1}_PC20.txt
else 
	pc1=/disk/reference_pgsfusion/1kg/${$ancestry1}/hm3_imp/${$ancestry1}_PC5.txt
fi
if [ "$ancestry2" = "EUR" ]
then
	pc2=/disk/reference_pgsfusion/1kg/${$ancestry2}/hm3_imp/${$ancestry2}_PC20.txt
else 
	pc2=/disk/reference_pgsfusion/1kg/${$ancestry2}/hm3_imp/${$ancestry2}_PC5.txt
fi
...
XPASS=/root/pgsfunsion/XPASS/XPASS.R
${Rscript} ${XPASS} --summ1 ${outpath}/summ1.txt --summ2 ${outpath}/summ2.txt \
                    --ref1 ${ref1} --ref2 ${ref2} \
                    --pc1 ${pc1} --pc2 ${pc2} \
                    --anc ${ancestry1} --output ${outpath}
